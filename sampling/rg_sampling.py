...
class ColouriastionRG(nn.Module):
    def __init__(self, channels_num, combined_channels, *args, **kwargs):
        ...
        self.rg = nn.Sequential(
            ...
            MaskedConv2d('B', channels_num, channels_num, 3, 1, 1), nn.BatchNorm2d(channels_num), nn.ReLU(True),
            MaskedConv2d('B', channels_num, channels_num, 3, 1, 1), nn.BatchNorm2d(channels_num), nn.ReLU(True),
            nn.Conv2d(channels_num, 256, 1)
        )

        self.gray = nn.Sequential(
            nn.Conv2d(1, channels_num, 3, 1, 1), nn.BatchNorm2d(channels_num), nn.ReLU(True),
...
def img_extended(img, gray):
    ...
    gray_extended = torch.empty(1, h, w * 2)
    gray_extended[:, :, 0::2] = gray
    gray_extended[:, :, 1::2] = gray
...
colouriastion_faces.load_state_dict(torch.load("models/less_rg.pth"))
...
print("RG without the additional channel")
shows()
